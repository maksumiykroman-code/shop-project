...
export default function AdminPage() {
  ...
  if (loading) return <div className="bg-black min-h-screen text-white p-10 font-sans">Завантаження...</div>;
    ...
    <div className="p-8 bg-black min-h-screen text-white font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid gap-4">
          {products.length === 0 ? (
            <p className="text-zinc-500">Товарів поки немає або вони завантажуються...</p>
          ) : (
            products.map((product) => (
              <div key={product.id} className="p-4 border border-zinc-800 rounded-xl bg-zinc-900/50 flex justify-between items-center">
                ...
                <div className="flex gap-3 text-xs font-bold uppercase tracking-tighter">
                  <button className="text-zinc-400 hover:text-white px-3 py-1 border border-zinc-700 rounded transition">Змінити</button>
                  <button className="text-red-500 hover:text-red-400 px-3 py-1 border border-zinc-700 rounded transition">Видалити</button>
                </div>
              </div>
            ))
          )}
